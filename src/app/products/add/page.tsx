...
function AddProduct() {
  ...
  const {
    ...
    onSubmit: async (values, actions) => {
      ...
      try {
        const response = await addProduct(values, categoryIds);
        alert(response.message);
        resetForm();
      } catch (error) {
        alert("Something went wrong, cannot add product");
      }
    },
  });
  ...
  function handleChangeSelect(selectedOptions, values) {
  ...
  function calculatePrice(oldPrice, discount) {
    ...
    return parseFloat(discountedPrice.toFixed(2)); // Ensure price is rounded to two decimal places
  }
